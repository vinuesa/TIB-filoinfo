...
progname=${0##*/}
version=0.2 # 2022-11-24; computes delta_BIC and BICw columns
...
echo "2. running in a for loop to combine all base models in model_set $model_set with (or not) +G and or +f"
for mat in "${models[@]}"; do
     ...
     printf -v model_string "%d\t%.5f\t%.5f\t%.5f" "$total_params" "$score" "$AICi" "$BICi"
...
done | sort -nk5 > "${infile}"_sorted_model_set_"${model_set}"_fits.tsv
...
declare -a BICw_a
BIC_a=( $(awk '{print $5}' "${infile}"_sorted_model_set_"${model_set}"_fits.tsv) )
min_BIC="${BIC_a[0]}"
...
if [[ -s "${infile}"_sorted_model_set_"${model_set}"_fits.tsv ]]; then
    ...
    awk 'BEGIN{print "model\tK\tlnL\tAIC\tBIC\tdeltaBIC\tBICw"}{print}' "${infile}"_sorted_model_set_"${model_set}"_fits.tsv | column -t
    awk 'BEGIN{print "model\tK\tlnL\tAIC\tBIC\tdeltaBIC\tBICw"}{print}' "${infile}"_sorted_model_set_"${model_set}"_fits.tsv > t
